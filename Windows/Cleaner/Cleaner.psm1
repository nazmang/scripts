...
Function Get-OrigFreeSpace{
    ...
    Try{
        $RawFreespace = (Get-WmiObject Win32_logicaldisk -ComputerName $ComputerOBJ.ComputerName  -ErrorAction Stop | Where-Object {$_.DeviceID -eq 'C:'}).freespace
        #-Credential $ComputerOBJ.Credential
        $FreeSpaceGB = [decimal]("{0:N2}" -f($RawFreespace/1gb))
        Write-host "Current Free Space on" $ComputerOBJ.ComputerName "OS Drive : $FreeSpaceGB GB" -ForegroundColor Yellow
...
Function Test-PSRemoting{
    ...
    $RemoteHostname = Invoke-command -ComputerName $ComputerOBJ.Computername -ScriptBlock {hostname}  -erroraction 'silentlycontinue'
    #-Credential $ComputerOBJ.Credential

    If($RemoteHostname -eq $ComputerOBJ.Computername){
        Write-Debug "PowerShell Remoting was successful" 
        $ComputerOBJ | Add-Member NoteProperty -Name PSRemoting -Value $True
    }
    Else {
        Write-Error "PowerShell Remoting FAILED on $ComputerOBJ.Name!" 
        ...
        Else{
            ...
            Invoke-Command -ComputerName $ComputerOBJ.ComputerName -ScriptBlock {
                ...
                Foreach($Folder in $Using:Paths){
                    If($i -gt 1){
                        Clear-Path -Path $Folder.fullname -ComputerOBJ $Using:ComputerOBJ
                    }
                    $i++
                }
    ...
    Else{
        ...
        Else{
            ...
            Foreach($Folder in $Paths){
                If($i -gt 1){
                    Clear-Path -Path $Folder.fullname -ComputerOBJ $ComputerOBJ  
                }
                $i++
            }
            Write-host "All Old Virus Definition Files cleaned successfully." -ForegroundColor Green
    ...
    If($ComputerOBJ.PSRemoting -eq $true){
        ...
        $CleanMGR = Invoke-command -ComputerName $ComputerOBJ.ComputerName -ScriptBlock {
                        ...
                        Try{
                            ...
                            Start-Process -FilePath Cleanmgr -ArgumentList '/sagerun:1 /autoclean' -Wait
                ...
                Foreach($Item in $LogPaths){
                    ...
                    If ($($LogFiles | Measure-Object).count -gt 0){ 
                        ForEach ($File in $LogFiles){ 
                            Remove-IISLogFile -ComputerOBJ $Computerobj -LogFile $File
                        } 
                    } 
                    ELSE{ 
...
Function Set-WindowsUpdateService{
    ...
    Write-Host "Deleting files from 'C:\Windows\SoftwareDistribution\Download'" -ForegroundColor Yellow
    ...
    If($ComputerOBJ.PSRemoting -eq $true){
        ...
        Else{
            ...
            Foreach($Profile in $Result){
                ...
                Clear-Path -Path $TempPath -ComputerOBJ $ComputerOBJ
                If($CleanDownloads -eq $true) { Clear-Path -Path $DownloadPath -ComputerOBJ $ComputerOBJ}
                ...
                Clear-Path -Path $GoogleChromeCache -ComputerOBJ $ComputerOBJ
                ...
                Clear-Path -Path $FirefoxCache -ComputerOBJ $ComputerOBJ               
            }
        }
    }
    Else{
...
.DESCRIPTION
   gets the used, free and total space with the drive name.
.EXAMPLE
...
Function Get-PurgeCount {
    Param
    (
        $ComputerOBJ,
        # Max Days For CCM Cache Entries
        $MaxRetention = 3
    )
